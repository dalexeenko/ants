...
By default, Ants uses a single bearer token for authentication. Multi-user mode adds individual user accounts with role-based access control.
...
:::note
`ANTS_MULTI_USER` and `ANTS_SECRET` are mutually exclusive. When multi-user mode is enabled, bearer token auth is replaced by user accounts.
:::
...
If `ANTS_SETUP_TOKEN` is not set, the first person to visit the setup page becomes the admin. This is fine for local development but not recommended for production.
...
```bash
ANTS_SETUP_TOKEN=$(openssl rand -base64 32)
```
