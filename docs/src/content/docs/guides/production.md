...
description: Best practices for running Ants in production — TLS, reverse proxy, security, and monitoring.
sidebar:
  order: 2
---
...
Before exposing Ants to the internet:

1. **Set a strong encryption key** — `openssl rand -base64 32`
2. **Set a stable secret** — `ANTS_SECRET` for a known bearer token, or enable multi-user mode
3. **Use HTTPS** — Put a reverse proxy in front for TLS termination
4. **Bind to all interfaces** — `ANTS_HOST=0.0.0.0`
5. **Configure CORS** — `ANTS_CORS_ORIGINS` for your app domains
6. **Restrict allowed hosts** — `ANTS_ALLOWED_HOSTS` for DNS rebinding protection
...
server {
    listen 443 ssl;
    server_name ants.yourdomain.com;
...
```bash
ANTS_CF_ACCESS_TEAM_DOMAIN=https://myteam.cloudflareaccess.com
ANTS_CF_ACCESS_AUD=your-application-audience-tag
```
...
- **API keys are encrypted at rest** with AES-256-GCM using the `ANTS_ENCRYPTION_KEY`
...
|------|----------|-------------|
| SQLite database | `ANTS_DATA_DIR` (default: `~/.config/ants-server/`) | All server state — projects, sessions, channels, etc. |
| Workspaces | `ANTS_WORKSPACES_DIR` (default: `~/ants/`) | Project files and agent outputs |
| Encryption key | `ANTS_ENCRYPTION_KEY` | Required to decrypt stored API keys |
