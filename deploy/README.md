...
Example infrastructure configurations for deploying the [OpenMgr](https://github.com/openmgr/openmgr) server.

## AWS (Terraform)

Deploys OpenMgr on **ECS Fargate with ARM64 (Graviton)** for lower cost.
...
> **Note:** You cannot set both `openmgr_secret` and `openmgr_multi_user`. They are mutually exclusive authentication modes.
...
> **Warning:** Without a custom domain, the server allows requests with **any** Host header (`OPENMGR_ALLOWED_HOSTS=*`), which disables [DNS rebinding](https://en.wikipedia.org/wiki/DNS_rebinding) protection. When you set `domain_name`, the allowed hosts are automatically restricted to that domain. For production, always configure a custom domain.
...
OpenMgr includes an optional full web app UI (served at `/app`) that provides a richer interface than the default server UI at `/ui`. It is disabled by default and can be enabled with:
...
When enabled, the `OPENMGR_WEB_APP=true` environment variable is passed to the container. The web app requires authentication — unauthenticated visitors are redirected to the login page.
...
docker tag openmgr/server:latest <ecr-repo-url>:latest
...
|---|---|---|
| `project_name` | Name prefix for all resources | `openmgr` |
| `environment` | Environment name | `production` |
| `aws_region` | AWS region | `us-east-1` |
| `image` | Docker image | `openmgr/server:latest` |
...
| `openmgr_encryption_key` | **Required.** AES-256 key for encrypting stored secrets | *(none)* |
| `openmgr_secret` | Bearer token (single-user mode). Mutually exclusive with `openmgr_multi_user`. | `""` |
| `openmgr_multi_user` | Enable multi-user mode. Mutually exclusive with `openmgr_secret`. | `false` |
| `openmgr_setup_token` | One-time setup token. Required when `openmgr_multi_user = true`. | `""` |
| `openmgr_web_app` | Enable the full web app UI at `/app` | `false` |
| `enable_ecr` | Create a private ECR repository | `false` |
| `ecr_repository_name` | ECR repository name | `openmgr/server` |
...
| `openmgr_env` | Extra env vars for the container | `{}` |
