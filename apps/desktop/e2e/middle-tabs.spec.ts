...
test.describe('Middle Tab Bar', () => {
  ...
  test('should show the chat tab', async () => {
    await expect(page.getByTestId('openmgr-middle-tab-chat')).toBeVisible();
  });
  ...
  test('should show terminal tab when opened', async () => {
    ...
    const addButton = page.getByTestId('openmgr-middle-tab-add');
    if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await terminalOption.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await expect(page.getByTestId('openmgr-middle-tab-terminal')).toBeVisible({ timeout: 5000 });
      }
    }
  });
  ...
  test('should switch back to chat tab after opening another', async () => {
    ...
    const chatTab = page.getByTestId('openmgr-middle-tab-chat');
    await chatTab.click();
    await expect(page.getByTestId('openmgr-chat-view')).toBeVisible({ timeout: 5000 });
  });
});
