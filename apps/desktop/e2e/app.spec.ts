...
/**
 * E2E tests for the Ants desktop app.
 *
...
test.describe('App Launch', () => {
  ...
  test('should show the welcome screen when no project is selected', async () => {
    ...
    await expect(page.getByTestId('ants-welcome-screen')).toBeVisible({ timeout: 10000 });
  });

  test('should have the main content area', async () => {
    await expect(page.getByTestId('ants-main-content')).toBeVisible();
  });
});
...
test.describe('Navigation via Icon Rail', () => {
  ...
  test('should navigate back to projects view', async () => {
    ...
    const welcomeVisible = await page.getByTestId('ants-welcome-screen').isVisible().catch(() => false);
    const sidebarVisible = await page.getByTestId('ants-project-sidebar').isVisible().catch(() => false);
