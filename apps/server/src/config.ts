...
export interface ServerConfig {
  ...
  corsOrigins: string[];
  /** Enable multi-user mode with RBAC (opt-in via ANTS_MULTI_USER=true) */
  multiUser: boolean;
  ...
  /** Enable the web app UI at /app (opt-in via ANTS_WEB_APP=true, default: false) */
  webApp: boolean;
...
function getDefaultDataDir(): string {
  return join(homedir(), '.config', 'ants-server');
}

function getDefaultWorkspacesDir(): string {
  return join(homedir(), 'ants');
}
...
function validateEncryptionKey(key: string): void {
  try {
    ...
    if (decoded.length !== 32) {
      console.error('Error: ANTS_ENCRYPTION_KEY must be 32 bytes (256 bits).');
      console.error('Generate one with: openssl rand -base64 32');
      process.exit(1);
    }
  } catch {
    console.error('Error: ANTS_ENCRYPTION_KEY must be valid base64.');
    console.error('Generate one with: openssl rand -base64 32');
    process.exit(1);
  }
}
...
export function loadConfig(): ServerConfig {
  const encryptionKey = process.env.ANTS_ENCRYPTION_KEY;
  
  if (!encryptionKey) {
    console.error('Error: ANTS_ENCRYPTION_KEY environment variable is required.');
    console.error('Generate one with: openssl rand -base64 32');
    process.exit(1);
  }
  ...
  const dataDir = process.env.ANTS_DATA_DIR || getDefaultDataDir();
  ...
  // ── Cloudflare Access ────────────────────────────────────────────────
  const cfAccessTeamDomain = process.env.ANTS_CF_ACCESS_TEAM_DOMAIN || config.cfAccessTeamDomain;
  const cfAccessAud = process.env.ANTS_CF_ACCESS_AUD || config.cfAccessAud;
  const cfAccessSetIdentity = process.env.ANTS_CF_ACCESS_SET_IDENTITY !== 'false' &&
  ...
  if ((cfAccessTeamDomain && !cfAccessAud) || (!cfAccessTeamDomain && cfAccessAud)) {
    console.error('Error: ANTS_CF_ACCESS_TEAM_DOMAIN and ANTS_CF_ACCESS_AUD must both be set.');
    process.exit(1);
  }

  // ── Multi-user (must be resolved before secret logic) ────────────────
  const multiUser = process.env.ANTS_MULTI_USER === 'true' || config.multiUser === true;
  ...
  const explicitSecret = process.env.ANTS_SECRET || config.secret;

  if (multiUser && explicitSecret) {
    console.error('Error: ANTS_SECRET cannot be used with multi-user mode.');
    console.error('In multi-user mode, authentication uses per-user credentials.');
    console.error('Remove ANTS_SECRET (or the "secret" config field) to continue.');
    process.exit(1);
  }

  // When CF Access is configured, don't auto-generate a secret — bearer
  // auth is only active if the user explicitly provides ANTS_SECRET.
  ...
  if (!multiUser && !secret && !hasCfAccess) {
    ...
    console.error('Either set ANTS_SECRET for bearer token auth, or set both');
    console.error('ANTS_CF_ACCESS_TEAM_DOMAIN and ANTS_CF_ACCESS_AUD for Cloudflare Access.');
    process.exit(1);
  }

  // ── Other config ─────────────────────────────────────────────────────
  // Port 0 is valid (OS assigns a free port), so use NaN check instead of falsy
  const envPort = parseInt(process.env.ANTS_PORT ?? '', 10);
  const port = !isNaN(envPort) ? envPort : (config.port ?? DEFAULT_PORT);
  const host = process.env.ANTS_HOST || config.host || DEFAULT_HOST;
  const workspacesDir = process.env.ANTS_WORKSPACES_DIR || config.workspacesDir || getDefaultWorkspacesDir();
  const agentPath = process.env.ANTS_AGENT_PATH || config.agentPath;
  const autoInstallAgent = process.env.ANTS_AUTO_INSTALL_AGENT !== 'false' && 
    (config.autoInstallAgent !== false);
  const mockAgent = process.env.ANTS_MOCK_AGENT === 'true' || config.mockAgent === true;
  const setupToken = process.env.ANTS_SETUP_TOKEN || config.setupToken || undefined;
  const pushContactEmail = process.env.ANTS_PUSH_CONTACT_EMAIL || config.pushContactEmail;
  const webApp = process.env.ANTS_WEB_APP === 'true' || (config as any).webApp === true;
  ...
  if (process.env.ANTS_ALLOWED_HOSTS) {
    allowedHosts = process.env.ANTS_ALLOWED_HOSTS.split(',').map(h => h.trim().toLowerCase());
  ...
  const sqliteJournalModeEnv = process.env.ANTS_SQLITE_JOURNAL_MODE?.toLowerCase();
  ...
  if (process.env.ANTS_CORS_ORIGINS) {
    corsOrigins = process.env.ANTS_CORS_ORIGINS.split(',').map(o => o.trim());
